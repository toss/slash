/**
 * isHookName 함수 수정을 제외하면 아래와 동일합니다.
...
/** hook 네이밍에 한글을 포함할 수 있도록 수정합니다 */
function isHookName(s) {
  return /^use[A-Z0-9ㄱ-ㅎ가-힣_].*$/.test(s);
}
...
function isForwardRefCallback(node) {
  return Boolean(node.parent && node.parent.callee && isReactFunction(node.parent.callee, 'forwardRef'));
}
...
function isMemoCallback(node) {
  return Boolean(node.parent && node.parent.callee && isReactFunction(node.parent.callee, 'memo'));
}
            ...
            if (isDirectlyInsideComponentOrHook) {
              ...
              if (!cycled && pathsFromStartToEnd !== allPathsFromStartToEnd) {
                const message =
                  ...
                  (possiblyHasEarlyReturn ? ' Did you accidentally call a React Hook after an' + ' early return?' : '');
                context.report({ node: hook, message });
              }
            } else if (
              codePathNode.parent &&
              (codePathNode.parent.type === 'MethodDefinition' || codePathNode.parent.type === 'ClassProperty') &&
